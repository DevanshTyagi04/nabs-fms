...
export function WorkflowStatusBadge({ status }: { status: string }) {
  ...
  if (status === 'ASSIGNED') return <Badge variant="info" dot>Assigned</Badge>;
  if (status === 'CANCELLED') return <Badge variant="neutral">Cancelled</Badge>;
  return <Badge variant="warning" dot>Created</Badge>;
}
...
export function WorkflowTimeline({ events }: { events: Array<{ id: string; description: string; timestamp: string; actor: string }> }) {
  ...
  return (
    <View style={[styles.timelineCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      <Text style={[styles.timelineTitle, { color: colors.cardForeground }]}>Audit & Workflow History</Text>
      <View style={{ gap: 10 }}>
        {events.map((evt) => (
          <View key={evt.id} style={styles.eventItem}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.evtMeta, { color: colors.mutedForeground }]}>By {evt.actor} • {evt.timestamp}</Text>
            </View>
          </View>
        ))}
      </View>
